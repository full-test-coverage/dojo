...
describe("#1 Kata - Move the cursor forward on matching", () => {
  ...
  it("should error if next goes beyound the size of input", () => {
    ...
    expect(() => parser.next()).toThrow(new ParsingError("End of input"));
  });
  ...
  it("should raise an error if expected character is not under the cursor", () => {
    ...
    expect(() => parser.expect("2")).toThrow(
      new ParsingError("Expected '2', got '1'")
    );
  });
  ...
  it("should raise an exception if it doesn't match in the middle", () => {
    ...
    parser.setInput("sweeeet");
    // When a string is expected
    expect(() => parser.expectString("sweeeef")).toThrow(
      new ParsingError("Expected 'f', got 't'")
    );
  });
});
