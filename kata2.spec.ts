...
describe("#2 Kata - Matching Expressions", () => {
    ...
    describe("or", () => {
        xit("should choose the first expression that succeeds", () => {
        ...
        xit("should choose properly reset the cursor after a failed attempt", () => {
            // Given a new base parser with some input
            const parser = new BaseParser();
            parser.setInput("abacate");
            // When faced with a list of options, where just the last one
        ...
        xit("should fail if no option succeeds", () => {
            // Given a new base parser with some input
            const parser = new BaseParser();
            parser.setInput("abacate");
    ...
    describe("oneOrMore", () => {
        xit("should raise an error if it doesn't match anything", () => {
            // Given a new base parser with some input
            const parser = new BaseParser();
            parser.setInput("bbbb");
            // When we try to parse the above input; Then it should throw an
            // error
            expect(() => parser.oneOrMore(() => parser.expect("a"))).toThrow(
                new ParsingError("Expected 'a', got 'b'")
            );
        });

        xit("should parse as many item out of the input as possible", () => {
            // Given a new base parser with some input
            const parser = new BaseParser();
            parser.setInput("aaab");
